...
CMD_FUNC(cmd_nick_local)
{
	...
	if (MyUser(client))
	{
		...
		if (match_spamfilter(client, spamfilter_user, SPAMF_USER, "NICK", NULL, 0, NULL, NULL))
			return;
	}
...
int _register_user(Client *client)
{
	...
	if (match_spamfilter(client, spamfilter_user, SPAMF_USER, NULL, NULL, 0, &savetkl, NULL))
	{
		if (savetkl &&
