...
COPY --chown=unrealircd:unrealircd ./ /install/
COPY --chown=unrealircd:unrealircd ./config.settings.docker /install/config.settings
...
RUN apt-get -yqq remove --purge \
    ...
    apt-get -yqq install libcurl4 libc-ares2 libsodium23 ca-certificates
